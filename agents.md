...
- Steps:
  ...
  4) Wait for Postgres and RabbitMQ to be healthy, apply EF migrations for Catalog, then start APIs.
  5) Run k6 perf tests (read/write) against `catalog-api` with thresholds enforced by `.github/scripts/assert-k6.js` (scripts live under `resources/k6/scripts` in-repo).
